...
describe('Rutas de Auth', () => {
    ...
    test('GET /me - Debe retornar usuario con token válido', async () => {
        const user = new User({
            name: 'Auth',
            lastName: 'Test',
            email: 'auth@test.com',
    ...
    test('GET /me - Debe fallar sin token', async () => {
        const response = await request(app)
            .get('/me')
            .expect(401);
    ...
    test('POST /logout - Debe cerrar sesión correctamente', async () => {
        const user = new User({
            name: 'Logout',
            lastName: 'Test',
