...
<h4 class="cursor-pointer text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="showHide">Show Guess Distribution</h4><div class="columns-1 justify-left m-2 text-sm dark:text-white">`
    ...
    for(let i=0; i<8; i++){
        let percentage = sum > 0 ? Math.ceil(winDistribution[i] / sum * 100) : 0;
        blocks += `<div class="flex justify-left m-1">
                        <div class="items-center justify-center w-2">${i+1}</div>
                        <div class="rounded-full w-full ml-2">
                            <div class="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 rounded-l-full" style="width: ${percentage}%;">
                                ${winDistribution[i]}
                            </div>
                        </div>
                    </div>`
